...
from external_module import register_bot, BOT_LIST
...
def turn():
    ...
    register_bot("1")
    dlog("BOT_LIST: "+str(len(BOT_LIST)))
    ...
    if robottype == RobotType.PAWN:
        ...
        if check_space_wrapper(row + forward, col + 1, board_size) == opp_team: # up and right
            ...
            upUpLeft = check_space_wrapper(row + forward + forward, col - 1, board_size) == opp_team
            upUpRight = check_space_wrapper(row + forward + forward, col + 1, board_size) == opp_team
            if not upUpLeft and not upUpRight: # up up and left
